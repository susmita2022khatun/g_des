...
function opt:adaptive_grad(theta, learning_rate, gradient, grad_scal_sum, theta_vel, time_stamp)
    ...
    for j = 1, 3 do
        theta[j] = theta[j] + learning_rate * gradient[j] * time_stamp * theta_vel[j] / math.sqrt(grad_scal_sum + EPSILON)
    end
...
function opt:rms_propagation(theta, learning_rate, gradient, grad_scal_sum_t_min_1 , decay_rate, theta_vel, time_stamp)
    ...
    for j = 1, 3 do
        theta[j] = theta[j] + learning_rate * gradient[j] * time_stamp * theta_vel[j] / math.sqrt(grad_scal_sum_t_min_1 + EPSILON)
    end
